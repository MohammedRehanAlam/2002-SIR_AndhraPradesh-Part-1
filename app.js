...
const DATABASE_DIR_NAME = 'Andhra Pradesh'; // Change this folder name here and everything updates automatically!
...
let neglectedFilesData = { skipped: [], corrupted: [], ignored: [] };
...
async function loadPreloadedDatabase(isAuto = false) {
  ...
  try {
    const res = await fetch(`${DATABASE_DIR_NAME}/manifest.json?t=` + Date.now());
    ...
    // Fetch indexing report if available
    try {
      const reportRes = await fetch(`${DATABASE_DIR_NAME}/search-index-report.json?t=` + Date.now());
      if (reportRes.ok) {
        const report = await reportRes.json();
        neglectedFilesData.skipped = report.skippedFiles || [];
        neglectedFilesData.corrupted = report.corruptedFiles || [];
        neglectedFilesData.ignored = report.ignoredFiles || [];
        console.log(`Loaded neglected files: ${neglectedFilesData.skipped.length} skipped, ${neglectedFilesData.corrupted.length} corrupted, ${neglectedFilesData.ignored.length} ignored.`);
      }
    } catch (e) {
      console.warn('Indexing report not found or empty.', e);
    }

    await onDatabaseLoaded('Pre-loaded Database');
  } catch { 
    if (!isAuto) manifestError?.classList.remove('hidden');
    else console.warn(`Auto-load: ${DATABASE_DIR_NAME}/manifest.json not found or empty.`);
  }
...
function buildTreeFromPaths(paths) {
  const root = { name: DATABASE_DIR_NAME, children: [], files: [] };
  for (const fp of paths) {
    const norm = fp.replace(/\\/g, '/');
    const rel = norm.startsWith(DATABASE_DIR_NAME + '/') ? norm.slice(DATABASE_DIR_NAME.length + 1) : norm;
...
function resetAppUI() {
  ...
  document.getElementById('indexing-strategy-section')?.classList.add('hidden');
  document.getElementById('neglected-section')?.classList.add('hidden');
  neglectedFilesData = { skipped: [], corrupted: [], ignored: [] };
  documentIndex.clear();
  isIndexed = false;
  currentMethod = null;
...
async function tryLoadPrecomputedIndex(scopeKeys) {
  ...
  try {
    ...
    let info = null;
    try {
      const infoRes = await fetch(`${DATABASE_DIR_NAME}/search-index-info.json?t=` + Date.now());
      if (infoRes.ok) {
    ...
    const isCompressed = (info && typeof info.compressed !== 'undefined') ? info.compressed : COMPRESSED_SEARCH_INDEXES;
    const extSuffix = isCompressed ? '.json.gz' : '.json';
    ...
    if (totalChunks > 0) {
      ...
      let chunksToLoad = [];
      if (info && info.fileToChunk && scopeKeys) {
        const neededChunks = new Set();
        scopeKeys.forEach(key => {
          if (info.fileToChunk[key] !== undefined) {
            neededChunks.add(info.fileToChunk[key]);
          }
        });
        chunksToLoad = Array.from(neededChunks).sort((a, b) => a - b);
        console.log(`Optimized load: Loading ${chunksToLoad.length} of ${totalChunks} chunks for this scope.`);
      } else {
        for (let i = 1; i <= totalChunks; i++) {
          chunksToLoad.push(i);
        }
        console.log(`Standard load: Loading all ${totalChunks} chunks.`);
      }

      if (chunksToLoad.length === 0) {
        console.log("No precomputed chunks contain files for this scope.");
      }
      ...
      chunksToLoad.forEach((chunkIdx) => {
        ...
        fetchPromises.push(
          (async () => {
            ...
            loadedChunks++;
            const pct = Math.round((loadedChunks / chunksToLoad.length) * 100);
            if (indexText) indexText.textContent = `Loading: ${loadedChunks}/${chunksToLoad.length} (${pct}%)`;
...
function sortTree(n) { n.children.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' })); n.files.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' })); n.children.forEach(sortTree); }
...
function updateSearchState() {
  const isEnabled = currentMethod !== 'Pre-loaded Database' || selectedPath.length >= MIN_FOLDER_DEPTH;
  if (searchInput) {
    searchInput.disabled = !isEnabled;
    searchInput.placeholder = isEnabled 
      ? "e.g. Mohammed Rehan Alam, ID No', Address.." 
      : `Please select at least ${MIN_FOLDER_DEPTH} folder${MIN_FOLDER_DEPTH > 1 ? 's' : ''} to enable search`;
  }
  if (findBtn) {
    findBtn.disabled = !isEnabled;
  }
}
...
function isPathInSelectedScope(filePath) {
  const normPath = filePath.replace(/\\/g, '/');
  let scopePrefix = DATABASE_DIR_NAME;
  if (selectedPath && selectedPath.length > 0) {
    scopePrefix += '/' + selectedPath.join('/');
  }
  return normPath.startsWith(scopePrefix + '/') || normPath === scopePrefix;
}
...
async function startSearch() {
  ...
  // Render and toggle neglected files banner
  const skippedInScope = neglectedFilesData.skipped.filter(f => f.path && isPathInSelectedScope(f.path));
  const corruptedInScope = neglectedFilesData.corrupted.filter(f => f.path && isPathInSelectedScope(f.path));
  const ignoredInScope = (neglectedFilesData.ignored || []).filter(f => f.path && isPathInSelectedScope(f.path));
  const totalNeglected = skippedInScope.length + corruptedInScope.length + ignoredInScope.length;

  const negSect = document.getElementById('neglected-section');
  const negTitle = document.getElementById('neglected-title');
  const negList = document.getElementById('neglected-list');

  if (totalNeglected > 0) {
    if (negTitle) negTitle.textContent = `Un-Readable Documents (${totalNeglected})`;
    if (negList) {
      const allNeglected = [
        ...skippedInScope.map(f => ({ ...f, type: 'Skipped' })),
        ...corruptedInScope.map(f => ({ ...f, type: 'Corrupted', reason: f.reason || f.error })),
        ...ignoredInScope.map(f => ({ ...f, type: 'Ignored' }))
      ];
      negList.innerHTML = allNeglected.map(f => {
        const reasonLabel = f.reason || 'Failed to read content';
        ...
        return `
          <div class="neglected-item">
            <div class="neglected-item-info">
              <span class="neglected-item-name" title="${escapeHtml(filename)}">${escapeHtml(filename)}</span>
              <span class="neglected-item-path">📂 ${escapeHtml(folderPath)}</span>
              <span class="neglected-item-reason">⚠️ ${f.type}: ${escapeHtml(reasonLabel)}</span>
            </div>
            <div class="neglected-item-action">
              <button class="btn-open" onclick="window.open('${escapeHtml(normPath)}', '_blank')">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> Open File
              </button>
            </div>
          </div>
        `;
    ...
    negSect?.classList.remove('hidden');
  } else {
    negSect?.classList.add('hidden');
  }
